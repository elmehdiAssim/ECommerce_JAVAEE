package com.ecommerce.controllers;

import com.ecommerce.beans.Produit;
import java.io.IOException;
import java.io.PrintWriter;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import org.ecommerce.dao.ProduitDB;

@WebServlet("/admin_interface/AjoutProduit")
public class AjoutProduit extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        try (PrintWriter out = response.getWriter()) {
            
            String designation = request.getParameter("designation");            
            String marque = request.getParameter("marque");
            String couleur = request.getParameter("couleur");            
            String connexion = request.getParameter("connexion");
            String typeBatterie = request.getParameter("typeBatterie");
            int prixAchat = Integer.parseInt(request.getParameter("prixAchat"));
            int prixVente = Integer.parseInt(request.getParameter("prixVente")); 
            int tauxRemise = Integer.parseInt(request.getParameter("tauxRemise"));
            int quantiteStock = Integer.parseInt(request.getParameter("quantiteStock")); 
            String imageLink = request.getParameter("imageLink");
            
            Produit produit = new Produit();
            
            produit.setDesignation(designation);           
            produit.setMarque(marque);
            produit.setCouleur(couleur);            
            produit.setConnexion(connexion);
            produit.setTypeBatterie(typeBatterie);
            produit.setPrixAchat(prixAchat);
            produit.setPrixVente(prixVente); 
            produit.setTauxRemise(tauxRemise);
            produit.setQuantiteStock(quantiteStock); 
            produit.setImageLink(imageLink); 
            
            ProduitDB.ajouter(produit);
            this.getServletContext().getRequestDispatcher("/admin_interface/ajouter_produit.jsp").forward(request, response);
           
            out.close(); 
                       
                      
            
            
        }
    }

    // <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    /**
     * Handles the HTTP <code>GET</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Handles the HTTP <code>POST</code> method.
     *
     * @param request servlet request
     * @param response servlet response
     * @throws ServletException if a servlet-specific error occurs
     * @throws IOException if an I/O error occurs
     */
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        processRequest(request, response);
    }

    /**
     * Returns a short description of the servlet.
     *
     * @return a String containing servlet description
     */
    @Override
    public String getServletInfo() {
        return "Short description";
    }// </editor-fold>

}
